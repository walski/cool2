...
describe "Cool2::Parser" do
  it "should collapse text nodes when cleaned" do
    text = "Bla"
    parser = Cool2::Parser.new('')
    
    def parser.mock_up!(text)
      @tree = Cool2::Tree::Root.new
      10.times do
        tree.content << Cool2::Tree::Text.new(text)
      end
    end
    ...
    tree = parser.tree
    tree.content.size.should be(10)
    parser.clean!
    tree.content.size.should be(1)
    tree.content.first.content.should ==(("#{text}\n" * 10).chomp)
  end
end